...
const player = {
    ...
    }, attack(direction) { //this could be cleaned up
    ...
    checkHit() {
        ...
        if (attackedEnemy) {
            gameEnemies.forEach((enemy) => {
                if (this.attackTarget[1] === enemy.position[1] && this.attackTarget[2] === enemy.position[2]) {
                    enemy.heart--;
                    if (enemy.heart <= 0) {
                        enemy.removeEnemy();
                    }
    ...
    checkTunnel: function () {
        ...
        if (playerInTunnel) { //function called every 1/10th second - if player in tunnel 
            for (let i = 0; i < tunnelLocations.length; i++)
                if (tunnelLocations[i][1] !== player.y && tunnelLocations[i][2] !== player.x) { //if player is not in this tunnel 
    ...
    matrixArray(range) {
        const r = range; const mVals = []; let col = 0; let row = 0; const length = ((2 * range) + 1) ** 2;
    ...
    move() {
        ...
        } else { console.log('player doesnt move') }
        this.interact();
        this.render('player');
    },
    pickLock() {
    ...
    pathIsClear(y, x) {
        ...
        const inTunnel = $(`#cell_${this.map}_${this.y + y}_${this.x + x}`).hasClass('tunnel')
        const foundTreasure = $(`#cell_${this.map}_${this.y + y}_${this.x + x}`).hasClass('treasure')
        if (inTunnel === true) {
