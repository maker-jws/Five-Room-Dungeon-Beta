...
function parseMap() {
    let n = 0;
    let t = 0
    for (let m = currentMap; m < mapPack.length; m++) { // revisit rather than rendering all maps in map pack just 
        for (let r = 0; r < mapPack[currentMap].length; r++) {
            ...
            for (let c = 0; c < mapPack[m][r].length; c++) {
                ...
                if (mapPack[m][r][c] === 1) { //wall 
                    ...
                    cell.addClass('floor');
                } else if (mapPack[m][r][c] === 3) { //locked door
                    cell.addClass('locked');
                } else if (mapPack[m][r][c] === 4) { //door
                    n++
                    doorLocations.push(["door:", r, c, m])
                    cell.addClass('door');
                } else if (mapPack[m][r][c] === 10) { //tunnel
                    t++
                    tunnelLocations.push([t, r, c, m])
                    cell.addClass('tunnel');
                } else if (mapPack[m][r][c] === 6) {//treasure
                    cell.addClass('treasure');
                } else if (mapPack[m][r][c] === 7) { //goal
                    cell.addClass('goal');
                } else if (mapPack[m][r][c] === "D") { //goal
                    cell.addClass('trapped');
                } else if (mapPack[m][r][c] === "L") { //lava
                    cell.addClass('lava');
                } else if (mapPack[m][r][c] === "W") { //Water
                    cell.addClass('water');
                } else if (mapPack[m][r][c] === "B") { //Bridge
                    cell.addClass('bridge');
                } else if (mapPack[m][r][c] === "A") { //Acid
                    cell.addClass('acid');
                } else if (mapPack[m][r][c] === "V") { //Void
                    cell.addClass('void');
                } else if (mapPack[m][r][c] === "T") { //Torch
                    cell.addClass('torch');
                } else if (mapPack[m][r][c] === "K") { //Torch
                    cell.addClass('key');
